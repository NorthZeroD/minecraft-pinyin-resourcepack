...
LANG = {
    "language.code": "zho-Hans_quanpin",
    "language.name": "汉语拼音",
    "language.region": "全拼",
}
...
def load_special_word_quanpin(path: str) -> dict[str, str]:
    mapping = {}
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
...
def main(PATHS: dict[str, str]) -> None:
    ...
    special_word_quanpin = load_special_word_quanpin(PATHS["special_word_quanpin"])
    pass_key = load_pass_key(PATHS["pass_key"])

    special_keys = sorted(special_word_quanpin.keys(), key = len, reverse = True)
    ...
    for k, v in lang_data.items():
        if k.startswith("item.minecraft.") or k.startswith("block.minecraft."):
            ...
            for sw in special_keys:
                if sw in v:
                    v = v.replace(sw, special_word_quanpin[sw])
